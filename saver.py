...
class MutliParamSaver(object):
    ...
    def ckpt_list(self) -> List[ckpt_t]:
        ...
        dir_tuple = [
            cast(ckpt_t, (str(r.group(0)), int(r.group(1)), float(r.group(2))))
            for r in dirs
            if r is not None
        ]
        return dir_tuple
    ...
    def load_ckpt(self, method) -> Tuple[Dict[str, Any], int, str]:
        if method == "highest" or method == "lowest":
            ...
            dir, step, score = self.load_by_step(int(method))
            assert step == int(method)
        else:
    ...
    def add_param(
        self, key: str, val: Union[torch.nn.Module, torch.optim.Optimizer]
    ) -> None:
        self.params[key] = val
...
class Saver(object):
    ...
    def ckpt_list(self):
        ...
        files = [
            (r.group(0), int(r.group(1)), float(r.group(2)))
            for r in files
            if r is not None
        ]
        return files
